...
\title{\vspace{-1em}Research Proposal}
...
The overarching research question is: which governance arrangements remain robust when interacting agents face adversarial turnover and oversight is imperfect?

This question has three concrete parts. First, how do governance rankings change as strategic pressure rises under repeated turnover? Second, when do hybrid or multi-level arrangements retain an advantage over purely central or purely local governance once oversight frictions are introduced? Third, how does the same evaluation framework extend to capability imbalance and to populations generated by language models?
...
The programme is cumulative in structure. Stage one is the commons benchmark under adversarial turnover and imperfect oversight. That stage is sufficient to stand as a PhD contribution because it delivers the main benchmark family and the main governance-ranking results. Stage two adds the LLM bridge as a sensitivity test within the same benchmark. Stage three extends the same evaluation logic to capability imbalance. This sequencing is intended to keep the project feasible while preserving a clear longer-term direction.

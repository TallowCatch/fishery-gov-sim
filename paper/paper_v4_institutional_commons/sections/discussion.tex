...
The institutional-friction results indicate that the ranking between hybrid and top-down-only governance depends on the scenario and on the practical limits of oversight. Community-irrigation settings favour hybrid governance most consistently. Constrained forest co-management and the higher-pressure regulated-fishery setting favour top-down-only governance.

These results narrow the scope of the baseline Harvest architecture result. The confirmatory architecture matrix ranks hybrid governance first under search-based pressure. After scenario structure and oversight frictions are included, that ordering is no longer uniform. The extension supports an interpretation based on scenario-specific rankings instead of a single architecture ordering.
